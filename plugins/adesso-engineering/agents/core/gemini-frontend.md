...
3. **Visual verification** (MANDATORY - DO NOT SKIP):
   You MUST visually verify the component in Chrome MCP before reporting completion.
   ...
   # 2. Navigate to test page (homepage or relevant page)
   mcp__claude-in-chrome__navigate(url="https://venneker.ddev.site/", tabId=<tabId>)

   # 3. Take screenshot to verify rendering
   mcp__claude-in-chrome__computer(action="screenshot", tabId=<tabId>)
   ```

   **CRITICAL**: Do NOT report "task complete" until you have:
   - Built the theme (`ddev theme build`)
   - Cleared cache (`ddev drush cr`)
   - Taken a screenshot showing the component rendered correctly

   If the visual result doesn't match expectations, iterate with Gemini to fix issues before reporting completion.
