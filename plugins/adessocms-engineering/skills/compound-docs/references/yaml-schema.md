...
- **component** (enum): One of [drupal_module, drupal_theme, drupal_entity, drupal_plugin, service_object, queue_worker, database, frontend_alpine, frontend_twig, email_processing, content_system, assistant, authentication, payments, development_workflow, testing_framework, documentation, tooling]
- **symptoms** (array): 1-5 specific observable symptoms
- **root_cause** (enum): One of [missing_reference, missing_cache_tag, missing_index, wrong_api, scope_issue, thread_violation, async_timing, memory_leak, config_error, logic_error, test_isolation, missing_validation, missing_permission, missing_workflow_step, inadequate_documentation, missing_tooling, incomplete_setup]
...
- **drupal_version** (string): Drupal version in X.Y.Z format
...
5. drupal_version (if provided) must match X.Y.Z format
...
component: drupal_entity
symptoms:
  - "N+1 query when loading paragraphs"
  - "Page generation taking >5 seconds"
root_cause: missing_cache_tag
drupal_version: 11.2.0
resolution_type: code_fix
severity: high
tags: [n-plus-one, cache-tags, performance]
---
```
